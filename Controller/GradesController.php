 ...
 */
		...
		foreach ($grades_split as $line) {
			...
			foreach ($line_split as $i => $grade) {
				...
				$existing = $this->Grade->find('all',
					array(
						'conditions' => compact('category_id', 'state_id', 'year')
					)
				);
    ...
    public function import_2020()
    {
        ...
        $file = new File(TMP . DS . 'ConexusScorecard2020.txt');
        $grades = $file->read();
    ...
    public function import_2018() {
        ...
        // Tab-delimited file
        App::uses('File', 'Utility');
        $file = new File(TMP.DS.'ConexusScorecard2018.txt');
        $grades = $file->read();
    ...
    public function import_2016() {
        ...
        // Tab-delimited file
        App::uses('File', 'Utility');
        $file = new File(TMP.DS.'ConexusScorecard2016.txt');
        $grades = $file->read();
	...
	public function import_2015() {
		...
		// Tab-delimited file
		App::uses('File', 'Utility');
		$file = new File(TMP.DS.'ConexusScorecard2015.txt');
		$grades = $file->read();
